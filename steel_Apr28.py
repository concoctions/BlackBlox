...
# turn individual tests on and off
...
BF_factory_dict = {
    'IBC-0C': dict(
        chain_list_file="data/steel/factories/IBF_factory-0C.xlsx",
    ...
    'IBC-LC': dict(
        chain_list_file="data/steel/factories/IBF_factory-LC.xlsx",
        chain_list_sheet='chains',
        connections_sheet='connections',
        name="BF-BOF-LC",
        scenario='BBF-0B',
    ...
    'IBC-HC': dict(
        chain_list_file="data/steel/factories/IBF_factory-HC.xlsx",
        chain_list_sheet='chains',
        connections_sheet='connections',
        name="BF-BOF-HC",
        scenario='BBF-0B',
...
DRI_factory_dict = {
    'DRI-0C': dict(
        chain_list_file="data/steel/factories/DRI_factory-0C.xlsx",
        chain_list_sheet='chains',
        connections_sheet='connections',
        name="DRI-EAF-0C",
        scenario='MID-0B',
    ...
    'DRI-LC': dict(
        chain_list_file="data/steel/factories/DRI_factory-LC.xlsx",
        chain_list_sheet='chains',
        connections_sheet='connections',
        name="DRI-EAF-LC",
        scenario='MID-0B',
    ...
    'DRI-HC': dict(
        chain_list_file="data/steel/factories/DRI_factory-HC.xlsx",
        chain_list_sheet='chains',
        connections_sheet='connections',
        name="DRI-EAF-HC",
        scenario='MID-0B',
...
if BF is True:
    ...
    com.test_factory_scenarios(
        factory_dict=BF_factory_dict,
        ...
        aggregate_flows=['CO2', 'CO2__upstream', 'CO2 removed', 'stored CO2', 'debt CO2', 'factory CO2',
                         'CH4 (CO2eq)', 'factory CH4', 'factory CO2 removed', 'CO2 removed__upstream (dry'],
...
if electricity is True:
    # electricity mix
    ibf_subdirs = ['No Biomass/BBF', 'No Biomass/TGR', 'No Biomass/HIS', 'High Biomass/BBF', 'High Biomass/TGR',
                   'High Biomass/HIS']
    ...
    for i in range(len(ibf_elec_scenarios)):
        ...
        inflow_dict, outflow_dict = com.test_factory_sensitivity(
            factory_dict=BF_factory_dict,
            ...
            variable='fueltype',
            variable_options=['electricity 0g/kWh', 'electricity 200g/kWh', 'electricity 400g/kWh',
                              'electricity 600g/kWh', 'electricity 800g/kWh', 'electricity 1000g/kWh', ],
...
if biomass_co2 is True:
    ...
    for i in range(len(dri_bio_scenarios)):
        ...
        inflow_dict, outflow_dict = com.test_factory_sensitivity(
            ...
            variable_options=['wood chips (EU no swiss, dry)', 'wood chips, RER emissions',
                              'wood chips, high emissions', 'wood chips, medium emissions'],
            scenario_product=False,
            scenario_unit=False,
            scenario_io=False,
            qty=qty,
...
if CO2_transport is True:
    transport_inflow_dict = iof.nested_dicts(3)
    transport_outflow_dict = iof.nested_dicts(3)
    ...
    for i in range(len(ibf_CO2transport_scenarios)):
        ...
        for flow in inflow_dict:
            for scen in inflow_dict[flow]:
                for fact in inflow_dict[flow][scen]:
                    transport_inflow_dict[flow][scen][fact] = inflow_dict[flow][scen][fact]

        for flow in outflow_dict:
            for scen in outflow_dict[flow]:
                for fact in outflow_dict[flow][scen]:
                    transport_outflow_dict[flow][scen][fact] = outflow_dict[flow][scen][fact]
    ...
    transport_dfs = [meta_df]
    transport_sheets = ["meta"]

    for flow in transport_inflow_dict:
        df = iof.make_df(transport_inflow_dict[flow])
        transport_dfs.append(df)
        transport_sheets.append(f"IN {flow}")

    for flow in transport_outflow_dict:
        df = iof.make_df(transport_outflow_dict[flow])
        transport_dfs.append(df)
        transport_sheets.append(f"OUT {flow}")

    iof.write_to_xls(
        df_or_df_list=transport_dfs,
...
if boiler_eff is True:
    ...
    for i in range(len(ibf_boiler_scenarios)):
        ...
        inflow_dict, outflow_dict = com.test_factory_sensitivity(
            ...
            variable_options=[0.6, 0.65, 0.7, 0.75, 0.8, 0.85,
                              0.9],
            scenario_product=False,
            scenario_unit=False,
            scenario_io=False,
