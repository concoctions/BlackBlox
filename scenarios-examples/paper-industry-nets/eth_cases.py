...
def eth_cases(product_name, factory_list, factory_names, f_suffix, f_kwargs, case_list, feedstock='', fuel=''):
    ...
    ##---------------------------------
        ...
        for j, c in enumerate(case_list):
            ...
            CO2_dict[case]['totals']['CO2 removed, total'] = multi_o.loc['contrib - produced CO2__bio', c.lower()] # CO2 produced at ethanol factory (from fermentation and steam generation)

            contrib_flows = [('CO2 emitted, biogenic (total)', 'CO2__bio'),
    ...
    product_filestr = 'ethanol'
    if 'stover' in product_name:
        product_filestr += '-stover'
    else:
        product_filestr += '-maize'
    ...
    for c in CO2_dict:
        ...
        # CO2 removals
        CO2_removed_feedstock.append((d['totals']['CO2 removed, total'] -  d[0]['CO2 produced, bio, of which heat'] - d[0]['CO2 emitted, biogenic, of which heat for CCS'] )  * -1)
    ...
    for c in CO2_dict:
        ...
        sankey = Sankey(ax=ax, scale=0.0001, offset=0.15, head_angle=90,
                        format='%.0f',)
        ...
        sankey.add(flows=c_in,
                ...
                edgecolor = '#1d2f3d',
                facecolor = '#1d2f3d',)  # Arguments to matplotlib.patches.PathPatch()
        sankey.add(flows=c_prod,
                labels=c_prod_labels,
                orientations=c_prod_or,
                label=f"{product_name} production",
                prior=0,
