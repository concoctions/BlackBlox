...
# Add for random input
    product = random.choice(unit1.inputs)
    while (product == 'fuel' and len(unit1.inputs) > 1):   #fuel combustion calcs writes CO2 to output in balance; this can mess up numbers if there's something else that writes to CO2. fix this later.
        product = random.choice(unit1.inputs)
    if product == 'fuel':
        product = unit1.varDF.at[s, 'fuelType']
    if  u1_in[product] > 0:
        qty = u1_in[product]
    else:
        qty = 1
...
# Add for random output
    product = random.choice(unit1.outputs)
    while (product == 'CO2' and len(unit1.outputs) > 1):   #fuel combustion calcs writes CO2 to output in balance; this can mess up numbers if there's something else that writes to CO2. fix this later.
        product = random.choice(unit1.outputs)
    if product == 'fuel':
        product = unit1.varDF.at[s, 'fuelType'] 
    if  u1_out[product] > 0:
